...
Each time one of these actions triggers a 'new block' event the recording is terminated, the recorded data added as a new line in the .csv file and a new recording is started. The saved data files are split by days and so the data directory will contain a seperate '.csv' file for each day of the recording.
...
#### Flagging
Each window name can be given a 'flag' (eg. 'work', 'break', 'writing'). This is partially automated by the 'update_flag_file' method in two ways:
...
The method will also create the 'flag_data.csv' file, which contains all the window names existing in the data with the final flags assigned. This file will be used in further analysis by the 'make_time_series' method (details in the next section). It can be edited manually, but it is not adviced, as changes will be overwritten by the 'update_flag_file' method the next time it will be used.
**Please note, that for practical purposes the seperator in the ".csv" files mentioned above cannot be '.' or ';'. By default '¤' (chr(164)) is used, as it cannot be written with a standard keyboard.**
